...
当前模型采用 K2=3.0×10^-7，为 pH 3–8、按最近 pH 路由的单 a* GBR。模型直接预测 HCrO4-、Cr2O7²-、CrO4²-，总 Cr(VI) 按 HCrO4- + 2×Cr2O7²- + CrO4²- 计算。模型文件哈希用于版本标识，外部测试指标来自模型包。confidence 为 pH 启发式评分，不是准确概率；训练浓度范围与颜色分布元数据缺失时显示未知。三个直接预测值执行非负约束后参与总量计算，代数守恒不证明预测准确。输入允许 pH 0–14 以报告域外情况，实际训练有效范围仍为 3–8。
