...
phase=input 时，仅返回 {"decision": "allow|clarify|off_topic|redirect_query|redirect_prediction"}。
...
phase=output 时，仅返回 {"decision": "allow|block"}。
...
def ask_tutor(prompt, history, mode, context, api_key, base_url, model,
    ...
    try:
        ...
        answer = provider_json(endpoint, api_key, model,
            build_messages(prompt, history, mode, context, learning_context, language), 1800, 60)
        labels = SCHEMAS[mode]
        ...
        reply = "Assistant unavailable or response validation failed. Check your credentials, model name and service status, then retry." if en else "助手服务暂时不可用或回答格式校验失败，请检查密钥、模型名称及服务状态后重试。"
        if mode == "prediction_analysis":
            reply += "\n\n" + notice
        return {"reply": reply, "configured": True, "model": model, "error": True}
